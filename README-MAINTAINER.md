...
Determine the upstream version (like `10.0.0`) and eventually update the
`package.json` file; the format is `10.0.0-pre`.
...
and fix them; assign them to a milestone (like `10.0.0`).
...
- add a new entry like _\* v10.0.0_
- commit with a message like _prepare v10.0.0_
...
```sh
xpm run install-all -C ~/Work/micro-os-plus/semihosting-xpack.git
xpm run test-all -C ~/Work/micro-os-plus/semihosting-xpack.git
```
...
- `npm dist-tag add @micro-os-plus/semihosting@10.0.0 latest`
- `npm dist-tag ls @micro-os-plus/semihosting`
